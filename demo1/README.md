...
- `Commission & Rules > Invoices`：按日期、Payment method、Payment type、Status、Brand 和关键词筛选发票，并提供详情/下载占位操作；
...
- Invoices 页面使用 `#1F2937` 作为 Payment ID、金额和标题等强信息文字，使用 `#374151` 作为表格正文，使用 `#4B5563` 作为表头、筛选标签和辅助说明；状态同时显示文字和圆点，不仅依靠颜色。
### 演示数据与支付安全
...
| 文件 | 职责 |
| --- | --- |
| `index.html` | Merchant 页面语义骨架、SVG 图标库、抽屉、状态选择器、活动页、归因规则页、佣金规则页、发票页和财务页 |
| `styles.css` | 视觉 Token、布局、组件、状态、动效和响应式样式 |
| `data.mjs` | 工作区、任务导航、时间范围快照、指标、伙伴、活动、归因规则、佣金规则、发票和财务模拟数据 |
| `app-core.mjs` | 不依赖 DOM 的状态函数 |
| `app.js` | 数据渲染、模块导航、活动筛选、归因模型交互、佣金规则筛选/详情交互、发票筛选/下载交互、财务趋势/付款交互、状态切换、抽屉和浏览器交互 |
